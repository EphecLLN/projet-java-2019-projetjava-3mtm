...
public class Silver extends Observable{
	Plateau tab = new Plateau();
	
public void  silver(int x,int y,int x2,int y2) {
	
	int [][] coord = {{x-1,y+1},{x+1,y},{x+1,y+1},
			{x-1,y-1},{x+1,y-1}};
	int [][] coord2 ={{x-1,y+1},{x-1,y},{x+1,y+1},
			{x-1,y-1},{x+1,y-1}};
	if(tab.tableau[x2][y2] == 0) {
	for(int i=0; i<5;i++) {
	if(((x2 == coord[i][0])&&(y2 ==coord[i][1]))||((x2 == coord[i][0])&&(y2 ==coord[i][1]))){
